...
func readCorpusREST(t *testing.T) blackboardREST {
	t.Helper()
	path := filepath.Join("..", "catalog", "agents", "knowledge-manager", "corpus-rest.yaml")
...
func TestBlackboardMemoryCompatibility(t *testing.T) {
	...
	for _, block := range []string{"corpus-ingest", "corpus-reader"} {
		...
		readKnowledgeYAML(t,
			filepath.Join("..", "catalog", "agents", "knowledge-manager", block, "declarations.yaml"),
			&declarations)
...
func TestBlackboardMemoryWriteBlock(t *testing.T) {
	...
	readKnowledgeYAML(t,
		filepath.Join("..", "catalog", "agents", "knowledge-manager", "memory-write", "tools.yaml"),
		&selection)
