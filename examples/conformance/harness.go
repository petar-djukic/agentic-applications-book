...
// harness.go at fork commit c87a5322 (GH-40). One substantive
// adaptation, declared: the fork builds the agent binary from a sibling
// agent-core source checkout (RequireCoreRoot); this copy builds
// agent-core/cmd/agent from the Go module the manifest pins, the same
// pattern the swarm demo uses, and drops --core-root because the
// knowledge-manager profiles carry no /opt/agent-core references.
// ProfilesRoot resolves to examples/catalog. readKnowledgeYAML is
// adopted from the fork's knowledge_manager_test.go. Everything else is
// unchanged.
...
// runtimePackage is the agent CLI inside the pinned runtime module.
...
// conformanceRoot returns this package's directory, independent of the
...
// ProfilesRoot returns the catalog root the shipped profiles live under
// (examples/catalog), independent of the test's working directory.
func ProfilesRoot() string {
	return filepath.Clean(filepath.Join(conformanceRoot(), "..", "catalog"))
}
...
// agentBinary builds the agent binary from the pinned runtime module once
// per test process and returns its path. The build runs in this module's
// directory so go.mod resolves the manifest's pinned agent-core version.
func agentBinary(t *testing.T) string {
	t.Helper()
	binaryOnce.Do(func() {
		out := filepath.Join(os.TempDir(), "examples-conformance-agent")
		build := exec.Command("go", "build", "-o", out, runtimePackage)
		build.Dir = conformanceRoot()
		var buf bytes.Buffer
		build.Stdout = &buf
		build.Stderr = &buf
		if err := build.Run(); err != nil {
			binaryErr = err
			t.Logf("build %s failed:\n%s", runtimePackage, buf.String())
			return
		}
		binaryPath = out
	})
...
func Run(t *testing.T, cfg RunConfig) RunResult {
	...
	args := []string{"--profile", profile, "--otel-log-file", logFile}
	if cfg.Directory != "" {
...
// readKnowledgeYAML reads and parses a YAML document for the blackboard
// tests (adopted from the fork's knowledge_manager_test.go).
