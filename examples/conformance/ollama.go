...
// ollama.go at fork commit c87a5322 (GH-40). Unchanged except the two
// skip messages, which name this repository's opt-in target instead of
// the fork's.
...
func liveModelGate(optIn bool, timeout time.Duration, model string, probe func(string) error) (time.Duration, string, error) {
	if !optIn {
		return 0, "live model conformance disabled; run `mage -d examples integration:blackboardmemory`", nil
	}
	...
	if err := probe(model); err != nil {
		return 0, fmt.Sprintf(
			"live model conformance enabled but dependency unavailable: %v; install/start the exact dependency and rerun `mage -d examples integration:blackboardmemory`",
			err,
		), nil
	}
	return timeout, "", nil
